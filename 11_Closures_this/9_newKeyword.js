...
function arrayFrom(arrayLikeObject) {
	//aslo called a as Array.prototype.slice.call(arrayLikeObject)
...
function invokeMax(fn, num) {
	...
	return function inner() {
		...
		counter++;
		//arguments referes to the arguments passed in the inner() function
...
function bind(fn, thisArg) {
	//Converting the object array into an array, only grabbing the first two parameters and leaving the other ones to the inner function
	var outerArgs =  [].slice.call(arguments,2)
	return function() {
			//Converts the remaining parameters into elements of an array, only grabbing those which are inserted after the first two parameters.
			var innerArgs = [].slice.call(arguments);
			//It concatenates the outerArgs and innerAtgs array
...
function flip(fn, thisArg) {
	...
	return function() {
			//Converts the remaining parameters into elements of an array, only grabbing those which are inserted after the first two parameters.
			var innerArgs = [].slice.call(arguments);
			//It concatenates the outerArgs and innerAtgs array, and creates a new array from :(first element unmany arguments - number of formal parameters in the function)
			var allArgs = outerArgs.concat(innerArgs).slice(0,fn.length)
			//reverses the order of the arguments in the array
